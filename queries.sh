...
echo "$($PSQL "SELECT distinct(name) FROM games inner join teams on winner_id = team_id or opponent_id = team_id where round='Eighth-Final' and year=2014 order by name")"
...
echo "$($PSQL "SELECT distinct(name) FROM games inner join teams on team_id = winner_id or team_id = opponent_id where name ilike 'Co%' order by name")"
